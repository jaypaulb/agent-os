...
process_conditionals() {
    ...
    local tracking_mode=${5:-"tasks_md"}  # Default to tasks_md if not provided
    local beads_enabled=${6:-"true"}  # Default to true if not provided
        ...
        if [[ "$line" =~ \{\{IF[[:space:]]+([a-z_]+)\}\} ]]; then
            ...
            case "$flag_name" in
                ...
                "compiled_single_command")
                    ...
                    ;;
                "tracking_mode_beads")
                    [[ "$tracking_mode" == "beads" ]] && condition_met=true
                    ;;
                "tracking_mode_tasks_md")
                    [[ "$tracking_mode" == "tasks_md" ]] && condition_met=true
                    ;;
                "beads_enabled")
                    [[ "$beads_enabled" == "true" ]] && condition_met=true
                    ;;
                *)
    ...
    while IFS= read -r phase_ref; do
        ...
        if [[ "$mode" == "embed" ]]; then
            ...
            if [[ -f "$source_file" ]]; then
                ...
                file_content=$(process_conditionals "$file_content" "${EFFECTIVE_USE_CLAUDE_CODE_SUBAGENTS:-true}" "${EFFECTIVE_STANDARDS_AS_CLAUDE_CODE_SKILLS:-true}" "true" "${EFFECTIVE_TRACKING_MODE:-tasks_md}" "${EFFECTIVE_BEADS_ENABLED:-true}")
...
compile_agent() {
    ...
    # EFFECTIVE_TRACKING_MODE, EFFECTIVE_BEADS_ENABLED
    # compiled_single_command=false for main file (will be true for embedded PHASE files)
    content=$(process_conditionals "$content" "${EFFECTIVE_USE_CLAUDE_CODE_SUBAGENTS:-true}" "${EFFECTIVE_STANDARDS_AS_CLAUDE_CODE_SKILLS:-true}" "false" "${EFFECTIVE_TRACKING_MODE:-tasks_md}" "${EFFECTIVE_BEADS_ENABLED:-true}")
...
load_base_config() {
    ...
    BASE_TRACKING_MODE=$(get_yaml_value "$BASE_DIR/config.yml" "tracking_mode" "tasks_md")
    BASE_BEADS_ENABLED=$(get_yaml_value "$BASE_DIR/config.yml" "beads_enabled" "true")
